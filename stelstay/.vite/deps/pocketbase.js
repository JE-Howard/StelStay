...
var RealtimeService = class extends BaseService {
  ...
  async subscribe(e2, t2) {
    ...
    const listener = function(e3) {
      ...
      let i2;
      try {
        i2 = JSON.parse(s2 == null ? void 0 : s2.data);
      } catch {
      }
      t2(i2 || {});
    };
    return this.subscriptions[e2] || (this.subscriptions[e2] = []), this.subscriptions[e2].push(listener), this.isConnected ? 1 === this.subscriptions[e2].length ? await this.submitSubscriptions() : (_a = this.eventSource) == null ? void 0 : _a.addEventListener(e2, listener) : await this.connect(), async () => this.unsubscribeByTopicAndListener(e2, listener);
  }
    ...
    if (this.hasSubscriptionListeners(e2)) {
      if (e2) {
        for (let t2 of this.subscriptions[e2])
          (_a = this.eventSource) == null ? void 0 : _a.removeEventListener(e2, t2);
        delete this.subscriptions[e2];
      } else
        this.subscriptions = {};
      this.hasSubscriptionListeners() ? this.hasSubscriptionListeners(e2) || await this.submitSubscriptions() : this.disconnect();
    }
  }
  ...
  async unsubscribeByTopicAndListener(e2, t2) {
    ...
    for (let i2 = this.subscriptions[e2].length - 1; i2 >= 0; i2--)
      this.subscriptions[e2][i2] === t2 && (s2 = true, delete this.subscriptions[e2][i2], this.subscriptions[e2].splice(i2, 1), (_a = this.eventSource) == null ? void 0 : _a.removeEventListener(e2, t2));
    s2 && (this.subscriptions[e2].length || delete this.subscriptions[e2], this.hasSubscriptionListeners() ? this.hasSubscriptionListeners(e2) || await this.submitSubscriptions() : this.disconnect());
  }
